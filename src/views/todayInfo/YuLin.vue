<template>
  <div class="todayInfo">
    ...
    <div class="refresh">
      <el-button type="warning" round @click="refresh">立即刷新</el-button>
    </div>

    <!-- 图形界面 -->
    <div class="garph">
      <el-tabs type="card" style="height: 90%;" @tab-click="handleClick">
        <el-tab-pane
          :label="items.label"
...
export default {
  ...
  mounted() {
    ...
    // websocket 初始化
    this.initWebSocket();
    this.$message.warning("点击选怎你要了解的参数状态");
  },
  beforeDestroy() {
    // 页面离开时断开连接，清除定时器
    this.disconnect();
  ...
  methods: {
    // 立即刷新
    refresh() {
      this.$axios.get("/forest/update").then(res => {
        if (res.status == 200) {
          // 建立连接对象
          let socket = new SockJS("http://106.15.227.37:8100/endpoint");
          // 获取STOMP子协议的客户端对象
          ...
          this.stompClient.connect(
            ...
            () => {
              this.stompClient.subscribe("/topic/forest", msg => {
                // 订阅服务端提供的某个 topic
                // 这里接收从服务器的数据
                this.tableData[0].data1 = msg[0] + "℃"; // 空气温度
                this.tableData[1].data1 = msg[1] + "%RH"; //空气湿度
                this.tableData[0].data2 = msg[2] + "ppm"; // 二氧化碳浓度
                this.tableData[1].data2 = msg[3] + "ppm"; // 烟雾浓度
                this.tableData[2].data1 = msg[4] + "℃"; // 水温
                this.tableData[0].data3 = msg[5] + "℃"; // 土壤温度
                this.tableData[1].data3 = msg[6] + "%RH"; // 土壤湿度
                this.tableData[0].data4 = msg[7] + "mg/L"; // 土壤盐分
                this.tableData[1].data4 = msg[8] + "cm/us"; // 土壤电导率
                this.tableData[2].data2 = msg[9] + "ntv"; // 浑浊度
              });
              this.$message.success('刷次成功');
            },
            err => {
              // 连接发生错误时的处理函数
              console.log("失败");
    ...
    handleClick(tab) {
      this.$axios
        .get("forest/standard?name=" + tab.$options.propsData.name)
        .then(res => {
          if (res.status == 200) {
    ...
    co2() {
      ...
      timeTicket = setInterval(function() {
        ...
        // 隐藏加载动画
        myChart.hideLoading();
        var co2 = localStorage.getItem("二氧化碳浓度");
        dataZ.push(co2);
        // 重新绘图
        myChart.setOption({
          xAxis: {
    ...
    wtem() {
      ...
      timeTicket = setInterval(function() {
        ...
        // 隐藏加载动画
        myChart.hideLoading();
        var wtem = localStorage.getItem("水温");
        dataZ.push(wtem);
        // 重新绘图
        myChart.setOption({
          xAxis: {
    ...
    whur() {
      ...
      timeTicket = setInterval(function() {
        ...
        // 隐藏加载动画
        myChart.hideLoading();
        var whur = localStorage.getItem("浑浊度");
        dataZ.push(whur);
        // 重新绘图
        myChart.setOption({
          xAxis: {
...
.el-tabs {
  background: url('../../assets/images/today.jpg') no-repeat center center;
